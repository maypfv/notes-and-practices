...
function evaluate(component, env) { 
    return is_literal(component)
           ...
           ? evaluate_logical_composition(component, env)
...
function is_logical_composition(comp) {
    return head(comp) === "logical_composition";
}

function logical_op(comp) {
    return list_ref(comp, 1);
}

function logical_lhs(comp) {
    return list_ref(comp, 2);
}
function logical_rhs(comp) {
    return list_ref(comp, 3);
}

function evaluate_logical_composition(comp, env) {
    if(logical_op(comp) === "&&") {
        return evaluate(logical_lhs(comp), env)
            ? evaluate(logical_rhs(comp), env) 
            : false;
    } else { // "||"
        return evaluate(logical_lhs(comp), env)
            ? true 
...
const primitive_functions = list(
       ...
       list("list",    list             ),
       list("parse",    parse             ),
       list("parse_and_evaluate",    parse_and_evaluate             ),
...
parse_and_evaluate("1+parse_and_evaluate('1+1;');");